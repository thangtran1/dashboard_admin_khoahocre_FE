...
import { MarketData } from "../public/dataExport";

const MarketPage = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 9;

  const totalPages = Math.ceil(MarketData.length / pageSize);
  const paginatedBlogs = MarketData.slice(
    ...
    <section className="w-full">
      ...
      <div className="xl:flex xl:gap-6">
        ...
        <div className="w-full xl:w-3/4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {paginatedBlogs.map((MarketData, idx) => (
            <div
              key={idx}
            ...
            >
              ...
              <div className="p-3">
                ...
                <div className="text-xs text-gray-500 flex items-center gap-2 mt-2">
                  <span>🏷 {MarketData.oldPrice}</span>
                  <span>📅 {MarketData.price}</span>
                </div>
                <Link
                  to={`/market/${MarketData.id}`}
                  state={{ MarketData }}
