import { MarketData } from "@/pages/user/public/dataExport";
...
export default function MarketDetail() {
  ...
  const MarketDatas = MarketData.find((b) => b.id === id);
  console.log("🚀 ~ YoutubeDetail ~ MarketDatas:", MarketDatas);

  if (!MarketDatas) return <div className="p-4">Không tìm thấy bài viết</div>;
        ...
        <aside className="order-1 lg:order-2 w-full lg:w-1/4 lg:sticky lg:top-24 h-fit">
          <div className="bg-background border rounded-xl shadow p-2 space-y-4">
            <div className="rounded-lg overflow-hidden">
              <img
                src={MarketDatas.image}
                alt={MarketDatas.title}
              ...
              <div className="py-1">
                ...
                <p className="text-sm text-gray-600 mt-1 line-clamp-2">
                  {MarketDatas.subTitle}
                </p>
                <div className="text-xs text-gray-500 flex items-center gap-3 mt-2">
                  <span>🏷 {MarketDatas.oldPrice}</span>
                  <span>📅 {MarketDatas.price}</span>
                </div>
              </div>
