...
export default function NewCoursesDetail() {
  const { id } = useParams();
  const average = 0;
  const total = 0;
  ...
  const NewCourseDatas = NewCourseData.find((b) => b.id === id);
        ...
        <aside className="order-1 lg:order-2 w-full lg:w-1/4 lg:sticky lg:top-24 h-fit">
          <div className="bg-background border rounded-xl shadow p-2 space-y-4">
            <div className="rounded-lg overflow-hidden">
              <img
                src={NewCourseDatas.image}
                alt={NewCourseDatas.title}
                className="w-full h-80 object-cover"
              />
              <div className="py-1">
                <h3 className="text-base font-semibold line-clamp-2">
                  {NewCourseDatas.title}
                </h3>
                <p className="text-sm text-gray-600 mt-1 line-clamp-2">
                  {NewCourseDatas.subTitle}
                </p>
                <div className="text-xs text-gray-500 flex items-center gap-3 mt-2">
                  <span>🏷 {NewCourseDatas.oldPrice}</span>
                  <span>📅 {NewCourseDatas.price}</span>
                </div>
              </div>
