import { aiCourses, Categories } from "@/pages/user/public/dataExport";
...
export default function ListTopicsDetail() {
  const { id } = useParams();
  const average = 0;
  const total = 0;
  ...
  const categorie = Categories.find((b) => b.id === id);

  if (!categorie) return <div className="p-4">Không tìm thấy bài viết</div>;
