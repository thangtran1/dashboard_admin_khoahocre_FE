...
import { GroupBuyData } from "../public/dataExport";

const GroupBuyPage = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 9;

  const totalPages = Math.ceil(GroupBuyData.length / pageSize);
  const paginatedBlogs = GroupBuyData.slice(
  ...
  return (
    <section className="w-full">
      ...
      <h2 className="text-2xl font-bold mb-6">
        Độc Quyền Group Buy Tại Khóa Học Rẻ
      </h2>

      <div className="xl:flex xl:gap-6">
        {/* Grid blog - chiếm 3 cột */}
        <div className="w-full xl:w-3/4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {paginatedBlogs.map((GroupBuyData, idx) => (
            <div
              key={idx}
              className="bg-background border shadow rounded-lg overflow-hidden"
            >
              <img
                src={GroupBuyData.image}
                alt={GroupBuyData.title}
                className="w-full h-56 object-cover"
              />
              <div className="p-3">
                <h3 className="text-base font-semibold line-clamp-2">
                  <Link to={`/group-buy/${GroupBuyData.id}`}>
                    {GroupBuyData.title}
                  </Link>
                </h3>
                <p className="text-sm text-gray-600 mt-1 line-clamp-2">
                  {GroupBuyData.subTitle}
                </p>
                <div className="text-xs text-gray-500 flex items-center gap-2 mt-2">
                  <span>🏷 {GroupBuyData.oldPrice}</span>
                  <span>📅 {GroupBuyData.price}</span>
                </div>
                <Link
                  to={`/group-buy/${GroupBuyData.id}`}
                  state={{ GroupBuyData }}
